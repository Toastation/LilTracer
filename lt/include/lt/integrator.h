...
class Integrator : public Serializable {
 public:
   ...
   */
  Integrator(const std::string &type) : Serializable(type) { n_sample = 1; };
...
class DirectIntegrator : public Integrator {
 public:
  DirectIntegrator() : Integrator("DirectIntegrator") { link_params(); };
...
class PathIntegrator : public Integrator {
 public:
  PathIntegrator() : Integrator("PathIntegrator"), depth(32) { link_params(); };
...
class AOIntegrator : public Integrator {
 public:
  AOIntegrator() : Integrator("AOIntegrator") { link_params(); };
  ...
  Spectrum render_pixel(Ray &r, Scene &scene, Sampler &sampler) {
    ...
    if (scene.intersect(r, si)) {
      ...
      for (int l = 0; l < nSample; l++) {
        ...
        if (!scene.intersect(rs)) s += Spectrum(1.) / (float(nSample));
      }
    }
