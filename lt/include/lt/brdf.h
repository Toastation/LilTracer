...
public:
    ...
    ShapeInvariantMicrosurface(const std::string& type, const Float& scale_x, const Float& scale_y) :
        Brdf(type)
    {
    ...
    vec3 to_stretched_space(const vec3& wi) {
...
public:
    ...
    static Float pdf(const vec3& wh) { return square_to_cosine_hemisphere_pdf(wh); }
    static vec3 sample_D(Sampler& sampler) { return square_to_cosine_hemisphere(sampler.next_float(),sampler.next_float()) ; }
};
...
public:
    GGXMicrosurface() :
        ShapeInvariantMicrosurface<SphereMicrosurface>("GGXMicrosurface", 0.1, 0.1)
    {
        link_params();
    }

    GGXMicrosurface(const Float& scale_x, const Float& scale_y) :
        ShapeInvariantMicrosurface<SphereMicrosurface>("GGXMicrosurface",scale_x, scale_y)
    {
        link_params();
    }
